...
void array_from_file(WeatherTable array, const char *filepath) 
{
    ...
    while (!feof(file)) 
    {
        ...
        if (k_year <= LST_YEAR && FST_YEAR <= k_year && 1 <= k_month && k_month <= MONTHS && FST_DAY <= k_day && k_day <= LST_DAY) /*Si los datos leidos estan dentro del rango esperado, genero el arreglo multidemnsional*/
        {
            array[k_year - FST_YEAR][k_month - 1][k_day - 1] = weather; /*Es necesario indexar de esta forma para lograr la carga de datos desde array[0][0][0]*/
        }
...
void array_dump_b(int a[], unsigned int length) 
{
    printf("[");
    if (length > 0) {
        printf("%d",a[0]);
