...
int main(int argc, char *argv[]) {
    ...
    int min_temp = min_tempR(array);
    max_temp_per_year(array,output);
    month_max_prec_per_year(array,outputB);


    /* show the ordered array in the screen */
    array_dump(array);
    printf("\nThe historical min temp of Cordoba, from the year 1980 to 2016 is %d\n",min_temp);
    printf("\nThe array wich has the max temp for each year is:\n");
    array_dump_b(output,YEARS);
    printf("\nThe month with max prec of each year are:\n");
    array_dump_b(outputB,YEARS);
